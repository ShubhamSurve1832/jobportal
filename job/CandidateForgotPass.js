...
import { CandidateForgotPassSchema } from "./validation/Schema";
  ...
  let { values, errors, touched, handleBlur, handleChange, handleSubmit } =
    useFormik({
      ...
      onSubmit: (values) => {
        let obj = {
          ...values,
          redirectUrl : `${baseulr}/job/ResetCandidatePortalPassword`
        }
    ...
    const ForgotPass = async(CandidateData) =>{
      try{
        let response = await axios.post(`${jobPortalBaseURL}user/forgot-password`,CandidateData,{
          headers : {
